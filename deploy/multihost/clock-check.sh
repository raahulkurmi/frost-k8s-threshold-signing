...
# Forces a time resync on each VM (restart systemd-timesyncd), waits for sync,
...
# Portable to macOS bash 3.2. Usage: clock-check.sh VM [VM ...]
# Can also be sourced: defines clock_check VM...
set -uo pipefail

_ck_now_ms() { perl -MTime::HiRes=time -e 'printf "%d\n", time()*1000'; }
_ck_on() { local vm="$1"; shift; perl -e 'alarm 60; exec @ARGV' multipass exec "$vm" -- "$@" </dev/null 2> >(cat >&2) | cat; return "${PIPESTATUS[0]}"; }

clock_check() {
  local max="${MAX_SKEW_MS:-1000}" vm bad=0 t0 t1 rv mid skew rtt tries synced
  for vm in "$@"; do
    _ck_on "$vm" sudo systemctl restart systemd-timesyncd || { echo "clock-check: $vm: cannot restart systemd-timesyncd" >&2; bad=1; continue; }
    synced=""

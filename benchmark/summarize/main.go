...
func main() {
	...
	if len(tags) > 1 {
		...
		for _, k := range keys {
			...
			for _, t := range tags {
				...
				fmt.Fprintf(&b, " RTT %s → %s / %s ms, **%.1f%% err**, %s req/s |", f1(s.rttQuorum), f1(s.median), f1(s.p95), 100*float64(s.errs)/float64(s.n), f1(s.throughput))
			}
			fmt.Fprintln(&b)
		}
		fmt.Fprintln(&b)
	...
	for _, s := range all {
		...
		fmt.Fprintf(&b, "| %s | %s | %s | %s | %s | %d | %d | %.1f | %s | %s | %s | %s | %s | %s | %s | %s |\n",
			map[bool]string{true: s.tag, false: "–"}[s.tag != ""], f1(s.rttQuorum), s.rttSource, cd, s.label, s.n, s.errs, 100*float64(s.errs)/float64(s.n),
			f1(s.median), f1(s.p95), f1(s.p99), f1(s.mean), f1(s.stddev), f1(s.min), f1(s.max), f1(s.throughput))
		if s.errs > 0 {

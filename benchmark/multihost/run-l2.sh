...
#
# T 3-of-5, strict, deadline 2 s, RSA-2048. Real WAN, no netem: one signer per
...
# measured RTT stopped, so the quorum needs the 3 far ones). Each: fan-out all and
# hedged, concurrency 1/10/50, N after WARMUP warm-up. Then the pod scale-up (T
# strict, fan-out all, 0 -> 50/100 pods, 3 reps) via scale-remote.sh on the
...
place() { _map_get "${HOST_PLACEMENT:-}" "$1" || echo unknown; }
{
  ...
  echo; echo "  ],"
  echo "  \"kubernetes\": \"$(on "$COORD_VM" kubectl --context kind-tk8s version -o json | jq -r .serverVersion.gitVersion)\", \"t\": 3, \"n\": 5, \"rsa_bits\": 2048, \"deadline\": \"2s\", \"verify_strategy\": \"strict\","
  echo "  \"n_requests\": $N, \"warmup\": $WARMUP, \"concurrency\": \"$CONCS\", \"fanouts\": \"$FANOUTS\", \"hedge_delay\": \"$HEDGE_DELAY\", \"scenarios\": \"$SCENARIOS\", \"far_quorum_stopped_signers\": \"$NEAREST\","
...
for sc in $SCENARIOS; do
  ...
  for f in $FANOUTS; do
    ...
    for c in $CONCS; do
      ...
      echo "  $sc $lab: quorum RTT (3rd nearest running signer) $(jq -r '[.[].rtt_ms | select(. != null)] | sort | .[2]' "$RES/$sc/$lab.rtt.json") ms; $(cat "$RES/$sc/$lab.metrics.json")"
      CSVS="$CSVS $sc-$f=$RES/$sc/$lab.csv"
    done
  done
done
signers_ctl start 1 2 3 4 5
...
if [[ $SCALE == 1 ]]; then
  echo "================ pod scale-up (T strict, fan-out all) ================"
  set_fanout all
  on "$COORD_VM" bash -lc "cd ~/tk8s && rm -rf /tmp/l2scale && benchmark/multihost/scale-remote.sh /tmp/l2scale && tar -C /tmp/l2scale -czf /tmp/l2scale.tgz scale"
  ...
  jq --slurpfile s "$RES/scale-per-rep.json" '. + {scale_up: {sizes: "50 100", reps: 3, t_variant: "strict, fanout all", per_rep: $s[0]}}' "$RES/env.json" > "$RES/env.json.tmp" && mv "$RES/env.json.tmp" "$RES/env.json"
fi
...
"$TOOLS/summarize" -title "Phase 7B Level 2: T 3-of-5 across 5 AWS regions" \
  -note "LABEL: $LABEL_TEXT. Coordinator $(place "$COORD_VM"); signers: $(for id in 1 2 3 4 5; do printf '%s ' "$id=$(place "$(id_vm "$id")")"; done). Tags: <scenario>-<fanout>; far-quorum = signers $NEAREST (lowest measured RTT) stopped. Configured netem column is always 0 (no netem); rows are labelled by the MEASURED quorum RTT during the run." \
